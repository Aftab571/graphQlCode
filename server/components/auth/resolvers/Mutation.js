...
const Mutation = {
  ...
  login: async (parent, { username, password }, context) => {
    ...
    let expiresIn = '30m';
    let authToken = sign({ userId: user.id }, APP_SECRET, { expiresIn });
    context.req.res.cookie('authToken', authToken);
    return {
      token: authToken,
      user
    };
  ...
  deleteUser: async (parent, { username, id }, context) => {
    ...
    if (toDeleteBy.username != undefined || toDeleteBy.id != undefined) return context.prisma.deleteUser(toDeleteBy);
