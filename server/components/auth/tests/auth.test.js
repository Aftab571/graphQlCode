...
describe('auth', () => {
  ...
  cases.forEach(obj => {
    ...
    test(`mutation ${id}`, async () => {
      if (obj.isTokenRequired) context.req.headers.authorization = 'Bearer ' + token['useradmin'];
  ...
  Object.values(authNegetiveCases).forEach(obj => {
    ...
    test(`query ${id}`, async () => {
      if (context.req) context.req.headers.authorization = 'Bearer ' + token['useradmin'];
      else context['req'] = { headers: { authorization: '' } };
      const output = await graphql(mockSchema, query, null, context);
      console.log(output);
